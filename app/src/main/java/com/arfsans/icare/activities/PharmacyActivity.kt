package com.arfsans.icare.activities
...
import com.arfsans.icare.R
import com.arfsans.icare.adapter.MainAdapter
import com.arfsans.icare.data.model.ModelResults
import com.arfsans.icare.viewmodel.ListResultViewModel
...
class PharmacyActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        listResultViewModel.setPharmacyLocation(strLokasi)
        progressDialog.show()
        listResultViewModel.getPharmacyLocation().observe(this, { modelResults: ArrayList<ModelResults>? ->
            if (modelResults?.size != 0) {
                mainAdapter.setResultAdapter(modelResults)
                progressDialog.dismiss()
            } else {
                Toast.makeText(this, "lokasi Apotek tidak ditemukan!", Toast.LENGTH_SHORT).show()
            }
        })
    }
